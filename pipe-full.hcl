...
# In fetch stage, we first add IISUBQ to "valid instructions" array to set it as a valid   
# after we add IISUBQ into the "need_regids" and need_valC arrays since operation needs one register 
# as one operand, and one immediate(constant) as the other operand, respectively. 
###
# In decode stage, to answer "What register should be used as the B source?" question, we used "rB" as the "srcB" (since the second operand is a register) and we did not use any decoded 
...
# In memory and program counter update stage we did not need any modification regarding IISUBQ
...
bool set_cc = (E_icode in {IISUBQ}) || E_icode == IOPQ &&
